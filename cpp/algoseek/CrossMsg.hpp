...
namespace algoseek {
    ...
    public:
        ...
        struct cross_msg_decoder : decoder_base, qi::grammar<Iterator, CrossMsg()>
        {
            cross_msg_decoder(timespec timestamp, char msgtype, int8_t side, int64_t order_id);

            qi::rule<Iterator, CrossMsg()> m_wire_msg; // member variables
        };
    ...
    template<typename Iterator>
    CrossMsg::cross_msg_decoder<Iterator>::cross_msg_decoder(timespec timestamp, char msgtype, int8_t side, int64_t order_id) :
        decoder_base(timestamp, msgtype, side, order_id),
        CrossMsg::cross_msg_decoder<Iterator>::base_type(m_wire_msg)
    {
        m_wire_msg = ( qi::as_string[ +qi::alnum ] >> "," >>
                       ...
                       qi::as_string[ *qi::alpha ] )  
                [qi::_val = phi::construct<CrossMsg> (
